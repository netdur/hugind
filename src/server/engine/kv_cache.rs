...
impl KvCacheManager {
    ...
    pub fn evict(&self, ctx: &mut Context, seq_id: i32, session_id: &str) -> Result<()> {
        let (disk_path, kv_head, n_keep, unified) = {
            ...
            let session = sessions
                .get(session_id)
                .ok_or(Error::BackendError("Session not found".to_string()))?;
            (
                session.disk_path.clone(),
                session.kv_head,
                session.n_keep,
                self.unified_memory_mode,
            )
        };
    ...
    pub fn restore(&self, ctx: &mut Context, seq_id: i32, session_id: &str) -> Result<usize> {
        ...
        if let Some(path) = &session.disk_path {
            ...
            if (n_tokens as usize) > session.tokens.len() {
                return Err(Error::BackendError(
                    "Disk state has more tokens than intent history".to_string(),
                ));
            }
        ...
        Err(Error::BackendError(
            "No state data found in RAM or Disk".to_string(),
        ))
    }
    ...
    pub fn save_to_disk(
        ...
        Self::write_state_file(
            &PathBuf::from(path),
            &state_data,
            session.kv_head as u32,
            session.n_keep as u32,
        )?;
