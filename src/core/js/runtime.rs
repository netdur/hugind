        ...
        self.context
            .with(|ctx| {
                let res: rquickjs::Result<()> = (|| {
                    ...
                    let get_args_fn_compat =
                        Function::new(ctx.clone(), move || args_json_clone.clone())?;
                    ...
                    if let Ok(default_export) = module.get::<_, Function>("default") {
                        ...
                        if let Some(promise) = result_val.as_promise() {
                            ...
                            let on_fail = Function::new(
                                ctx.clone(),
                                move |_ctx: rquickjs::Ctx, err: rquickjs::Value| {
                                    *output_clone.lock().unwrap() =
                                        Some(Err(rquickjs::Error::new_loading_message(
                                            "Async Error",
                                            format!("{:?}", err),
                                        )));
                                    Ok::<(), Error>(())
                            ...
                            let _ = then_fn.call::<_, rquickjs::Value>((
                                rquickjs::function::This(promise.as_object().unwrap().clone()),
                                on_success,
                                on_fail,
                            ))?;
                        } else {
                            let json =
                                js_to_json(&ctx, result_val).unwrap_or(serde_json::Value::Null);
...
fn js_to_json<'js>(
    ...
    if value.is_null() || value.is_undefined() {
        ...
        Ok(serde_json::Value::Number(
            serde_json::Number::from_f64(value.as_number().unwrap()).unwrap(),
        ))
