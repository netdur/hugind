...
''';
    ...
    // Get titles
    final List<String> titles = extractSearchTitles(searchJson, sys);
    final String titlesStr = '1';
    sys.print(titlesStr);
    sys.print('2');
...
List<String> extractSearchTitles(String jsonText, dynamic sys) {
  ...
  try {
    ...
    if (query is Map) {
      ...
      if (search is List) {
        // Workaround for dart_eval loop bugs: use recursion returning list
        titles = _extractRecursive(search, 0, sys);
      }
    }
    sys.print("Titles: ${titles.length}");
  } catch (e) {
    sys.print('Error parsing search results: $e');
  }
  return titles;
}

List<String> _extractRecursive(List search, int index, dynamic sys) {
  if (index >= search.length) return <String>[];

  // Dynamic debug print (Stable)
  sys.print("Check 1: $index");

  final currentList = _extractRecursive(search, index + 1, sys);
  final item = search[index];

  if (item is Map) {
    final title = item['title'];
    if (title != null) {
      currentList.insert(0, "$title");
      // Dynamic debug print (Stable)
      sys.print("Check 2: Found $title");
    }
  }
  return currentList;
}
